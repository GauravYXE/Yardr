...
export default function AddGarageSaleScreen() {
  ...
  const searchAddress = async (text: string) => {
    if (text.length >= 3) {
      try {
        ...
        const results = await Location.geocodeAsync(text + ', Saskatchewan, Canada');
        ...
        if (results.length > 0) {
          ...
          const validSuggestions = suggestions.filter(s => s !== null) as Array<{
            address: string;
            latitude: number;
            longitude: number;
          }>;
  ...
  const handleVideoRecorded = async (recordedVideoUri: string, frames: string[]) => {
    setShowCamera(false);
    setAnalyzing(true);
  ...
  const selectSuggestion = (suggestion: { address: string; latitude: number; longitude: number }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!form.startDateTime) {
      Alert.alert('Error', 'Please enter start date and time (YYYY-MM-DD HH:MM)');
      return;
    }
    if (!form.endDateTime) {
    ...
    try {
      ...
      if (!rateCheck.allowed) {
        setLoading(false);
        Alert.alert('Posting Limit Reached', rateCheck.message || 'You have reached the posting limit.');
        return;
      }
      ...
      <View style={styles.container}>
        <ThemedView style={styles.analyzingContainer}>
          ...
          <View style={styles.stepsContainer}>
            ...
            <ThemedText style={styles.step}>⏳ Getting your location...</ThemedText>
        ...
        {videoUrl && (
          <View style={styles.aiNotice}>
            <ThemedText style={styles.aiNoticeText}>
              ✨ Title, description, and address were auto-filled by AI. Please review and complete the remaining fields.
            </ThemedText>
          </View>
        )}
